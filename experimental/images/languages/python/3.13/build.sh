...
# Download and compile Python 3.13.11
wget https://www.python.org/ftp/python/3.13.11/Python-3.13.11.tgz && \
    ...
    make altinstall && \
    cd .. && \
    rm -rf Python-3.13.11 Python-3.13.11.tgz && \
...
if [ "$L10N" = "zh_CN" ]; then
    pip3.13 config set global.index-url https://mirrors.tuna.tsinghua.edu.cn/pypi/web/simple || true
fi
